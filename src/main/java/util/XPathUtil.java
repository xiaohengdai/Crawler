...
public class XPathUtil {
    ...
    public static void showFailure(){
        ...
        log.info("clickFailureElementList中的size:"+size);
        if(size !=0){
        ...
        log.info("xpathNotFoundElementList中的size:"+size);
        if(size != 0){
    ...
    public static void initialize(String udid){
        ...
        if(Util.isAndroid(udid)){
            ...
            if(androidBottomBarID != null) {
                ...
                tabBuilder = new StringBuilder("//*[" + androidBottomBarID +"]/descendant-or-self::*[@clickable=\"true\"]");//descendant-or-self  选取当前节点的所有后代元素（子、孙等）以及当前节点本身
            ...
            if(iosBottomBarId != null){
                tabBuilder = new StringBuilder("//*[@visible=\"true\" and ancestor-or-self::" + iosBottomBarId +"]") ; //ancestor-or-self  选取当前节点的所有先辈（父、祖父等）以及当前节点本身
...
//                clickBuilder.append(" (descendant-or-self::" + item + ")");
        ...
        log.info("利用Driver自带的pageSourceDriver.driver.getPageSource3():"+Driver.driver.getPageSource());
        log.info("利用二次封装的Driver.getPageSource3():"+Driver.getPageSource());
        deviceHeight = Driver.getDeviceHeight();
        deviceWidth = Driver.getDeviceWidth();
        log.info("利用Driver自带的pageSourceDriver.driver.getPageSource4():"+Driver.driver.getPageSource());
        log.info("利用二次封装的Driver.getPageSource4():"+Driver.getPageSource());
    ...
    public static PackageStatus isValidPackageName(String packageName, boolean restart){
        ...
        //包名不合法检查app是否Crash(指不在VALID_PACKAGE_LIST中)
        log.info("isValidPackageName中isValid:"+isValid);
        ...
        while(--length >= 0 && !stop){
            ...
            if(set.add(nodeXpath)){
                ...
                if(null == elem){
                    ...
                    log.info("利用Driver自带的pageSourceDriver.driver.getPageSource1():"+Driver.driver.getPageSource());
                    break;
                }
