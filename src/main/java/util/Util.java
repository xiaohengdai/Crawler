...
public final class Util {
    ...
    public static boolean isProcessExist(String udid, String processName) {
        ...
        if (Util.isAndroid(udid)) {
            ...
            String res="";
            res = Util.exeCmd("idevicecrashreport -u " + udid + "  " + reportDir).toLowerCase();
            log.info("res:"+res);
            log.info("processName:" + processName);
            log.info("res.contains(processName):" + res.contains(processName));
...
////
////            if (res.contains(processName)) {
////                //Crash report中找到了ipa的名字，说明在此时执行自动化之前就已经有crash了
